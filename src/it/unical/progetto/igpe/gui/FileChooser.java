...
public class FileChooser {
	...
	public FileChooser(MainFrame frame, boolean online) {
		this.frame = frame;
	...
	public boolean functionSaveFile(int p1, int p2) {
		...
		if (value == JFileChooser.APPROVE_OPTION) {
			...
			if (file.exists() && existFile(filesInDirectory, filename)) {
				int result = pane.showConfirmDialog(frame, "The file name exists. Please change name",
	...
	public boolean functionLoadFile() {
		...
		if (value == JFileChooser.APPROVE_OPTION) {
			...
			if (!existFile(filesInDirectory, filename)) {
				int result = JOptionPane.showConfirmDialog(frame, "IL FILE NON ESISTE", "New File Name",
