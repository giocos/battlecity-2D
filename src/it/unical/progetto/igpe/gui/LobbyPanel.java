...
import it.unical.progetto.igpe.net.Server;
...
public class LobbyPanel extends JPanel {
	...
	private ClientChatPanel client;
	private Timer timer;
	...
	private List<JRadioButton> level;
	private List<JButton> buttons;
	
	public LobbyPanel(int w, int h, PanelSwitcher switcher) {
	...
	public class MyTask extends TimerTask {
		...
		public void run() {
			if (client.isReadyP1() && client.isReadyP2()
					&& client.getClientName().equals(client.getNameOfClientsOnline().get(0))) {
				try {
			...
			if (countDown < 0) {
				try {
					client.getDout().writeUTF(client.getPoints() + "StartGame");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	public void createDifficultPanel() {
		...
		for (int i = 0; i < 3; i++) {
			...
			if ((!client.isReadyP1() || !client.isReadyP2()) && client.getNameOfClientsOnline().size() > 0
					&& client.getClientName().equals(client.getNameOfClientsOnline().get(0)))

				level.get(i).addActionListener(e -> {
					if (!client.isReadyP1() || !client.isReadyP2()) {
			...
			if ((client.isReadyP1() && client.isReadyP2()) || client.getNameOfClientsOnline().size() > 1
					&& client.getClientName().equals(client.getNameOfClientsOnline().get(1))) {
	...
	public void createChat(ClientChatPanel client) {
	...
	public void createMapsPanel() {
		...
		if (client.getNameOfClientsOnline().size() > 1
				&& client.getClientName().equals(client.getNameOfClientsOnline().get(1))) {
	...
	public void setClient(ClientChatPanel client) {
		this.client = client;
	}
	
	public ClientChatPanel getClient() {
		return client;
	}
}
