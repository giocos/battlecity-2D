...
public class PlayerPanel extends JPanel {
	...
	private static final int POSY = 70;
	private static final int POSX = 120;
	...
	private class MyListener extends MouseInputAdapter {
		...
		public void mousePressed(MouseEvent e) {
			if (e.getComponent().getY() == buttons.get(0).getY()) {
				cursorPosition = 0;
			} else if (e.getComponent().getY() == buttons.get(1).getY()) {
					cursorPosition = 1;
			} else if (e.getComponent().getY() == buttons.get(2).getY()) {
					cursorPosition = 2;
			}
			repaint();
		}
