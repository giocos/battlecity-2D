...
public class SlideStage extends JLayeredPane {
	...
	// ONLINE
	public SlideStage(int w, int h, PanelSwitcher switcher, JTextField filename,
					  ConnectionManager connectionManager, String difficult) {
	...
	private void instantiateOnline(String difficult, ConnectionManager connectionManager) {
		...
		((MainFrame) switcher).setGameManager(((MainFrame) switcher).getGamePanel()
				.startNetwork(connectionManager, filename));
		((MainFrame) switcher).getGamePanel().setGame(((MainFrame) switcher).getGameManager());
		((MainFrame) switcher).setFullGamePanel(new FullGamePanel(WIDTH, HEIGHT,
				((MainFrame) switcher).getGameWidth(), ((MainFrame) switcher).getGameHeight(), getSwitcher(),
				((MainFrame) switcher).getGamePanel()));
	}
...
//			((MainFrame) switcher).getGameManager().setMedium(true);
		...
		((MainFrame) switcher).setFullGamePanel(new FullGamePanel(WIDTH, HEIGHT, ((MainFrame) switcher).getGameWidth(),
				((MainFrame) switcher).getGameHeight(), ((MainFrame) switcher), ((MainFrame) switcher).getGamePanel()));
		((MainFrame) switcher).getGamePanel().setFullGamePanel(((MainFrame) switcher).getFullGamePanel());
	}
